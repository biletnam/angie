 ...
 * every site. Since we have to ask the user to modify such values, we have to read them all and show them all to the user
 */
...
PHP;
        ...
        while(!$error)
        {
            try
            {
                $info = $tokenizer->searchToken('T_VARIABLE', '$settings', $skip);
                $skip = $info['endLine'] + 1;

                $settings = $this->extractVariables($info['data'], 'settings');

                // Good, I have found the correct line containing the hash salt
                if(is_array($settings) && isset($settings['hash_salt']))
                {
                    $hash_line = $info['startLine'];
        ...
        // Do I have the start-end lines? Good, now let's replace them with our code
        if($hash_line)
        {
            $random   = new AUtilsRandval();
            $new_salt = '$settings[\'hash_salt\'] = \''.str_replace(array('+', '/', '='), array('-', '_', ''), base64_encode($random->generate(55))).'\';';
            $out      = $tokenizer->replaceToken('T_VARIABLE', '$settings', $hash_line, $new_salt);
    ...
    public function writeConfig($file)
    {
        ...
        // System settings. Since it's a serialized array, I have to load it, change it and then update the object
        ...
        if($config !== false)
        {
            $config['name'] = $this->get('sitename', null, $key);
            $config['mail'] = $this->get('site_mail', null, $key);

            $query = $db->getQuery(true)
                        ->update($db->qn('#__config'))
                        ->set($db->qn('data').' = '.$db->q(serialize($config)))
                        ->where($db->qn('name') . ' = ' . $db->q('system.site') );
        ...
        $query = $db->getQuery(true)
                    ->select($db->qn('data'))
                    ->from('#__config')
                    ->where($db->qn('name') . ' = ' . $db->q('system.file') );
        $serialized = $db->setQuery($query)->loadResult();
        ...
        if($config !== false)
        {
            ...
            $config['path']['temporary'] = $this->get('tmp_path', sys_get_temp_dir(), $key);
        ...
        // Update CRON key
        $query = $db->getQuery(true)
                    ->update($db->qn('#__key_value'))
                    ->set($db->qn('value') . ' = ' . $db->q(serialize($this->get('cron_key', null, $key))))
                    ->where($db->qn('name') . ' = ' . $db->q('system.cron_key'));
        $db->setQuery($query)->execute();
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->qn('name') . ' = ' . $db->q('system.private_key'));
        $db->setQuery($query)->execute();
